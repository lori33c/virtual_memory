...
std::list <page_node> lru_list[TLB_SIZE];
...
void tlb_hit(size_t& frame, size_t& page, size_t& tlb_hits, int result) { 
    // // frame number is obtained from the TLB
...
void fifo_replace_page(size_t& frame ) {
    ...
    tlb_remove(frame);
    // replace 
    update_frame_ptable(pg_table[index].npage, frame);
    frame = pg_table[index].frame_num;
    tlb_add(frame, pg_table[index]);
 }   

void lru_replace_page(size_t& frame) {
    // replaces the least recently used pages
 } // TODO
